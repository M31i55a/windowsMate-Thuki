...
use super::defaults::{
    ...
    BOUNDS_TTS_RATE, BOUNDS_APP_BG_OPACITY, DEFAULT_AGENT_PROVIDER,
    ...
    DEFAULT_TTS_VOICE, DEFAULT_COLOR_PRIMARY, DEFAULT_APP_BG_OPACITY, SLASH_COMMAND_PROMPT_APPENDIX,
};
...
pub(crate) fn resolve(config: &mut AppConfig) {
    ...
    // Appearance section.
    if config.appearance.color_primary.trim().is_empty() {
        config.appearance.color_primary = DEFAULT_COLOR_PRIMARY.to_string();
    }
    clamp_f64(
        &mut config.appearance.app_bg_opacity,
        BOUNDS_APP_BG_OPACITY,
        DEFAULT_APP_BG_OPACITY,
