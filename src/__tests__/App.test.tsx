...
describe('App', () => {
  ...
  it('renders the no-models strip copy when Ollama is reachable but empty (S2)', async () => {
    ...
    expect(strip.textContent).toContain(
      "Mate couldn't find any local LLM models",
    );
  ...
  it('shows overlay on visibility show event', async () => {
    ...
    expect(
      screen.getByPlaceholderText('Ask Mate anything...'),
    ).toBeInTheDocument();
  });
  ...
  it('hides overlay on Escape key and cancels an active /search turn', async () => {
    ...
    expect(invoke).toHaveBeenCalledWith('notify_overlay_hidden');
    expect(screen.queryByPlaceholderText('Ask Mate anything...')).toBeNull();
    vi.useRealTimers();
  });
  ...
  it('enters hiding state on hide-request visibility event', async () => {
    ...
    await act(async () => {
      emitTauriEvent('mate://visibility', { state: 'hide-request' });
    });
  ...
  describe('image integration', () => {
    ...
    it('cancels deferred submit when all images fail', async () => {
      ...
      expect(screen.getByPlaceholderText('Ask Mate anything...')).toHaveValue(
        'describe',
      );
    });
  });
  ...
  describe('Onboarding', () => {
    it('shows onboarding screen when mate://onboarding event fires', async () => {
      enableChannelCaptureWithResponses({
      ...
      await act(async () => {
        emitTauriEvent('mate://onboarding', { stage: 'permissions' });
      });
    ...
    it('dismisses onboarding and shows ask bar when onComplete is called', async () => {
      ...
      await act(async () => {
        emitTauriEvent('mate://onboarding', { stage: 'intro' });
      });
