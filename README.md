...
- **Chat bubble color:** pick any accent color for your chat bubbles — scrollbar, slider controls, and UI highlights adapt automatically
...
| **Chat bubble color picker**      | Choose any accent color; scrollbar, slider controls, and UI highlights update in real time                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **Clear session (Ctrl+K)**        | Press <kbd>Ctrl</kbd>+<kbd>K</kbd> while Mate is open to instantly clear the chat, attached images, and query — start a fresh session without reopening the window                                                                                                                                                                                                                                                                                                                         |
| **Drag-to-select region capture** | Type `/screen region select` or click the crosshair button in the ask bar; a fullscreen overlay opens with a crosshair cursor. Drag a rectangle over any screen area, release, and the cropped region is attached as context. Escape or right-click cancels. Add optional text after `region select` (e.g., `/screen region select explain`) and it will be injected into the input for review before sending. Works alongside `/screen` (full-screen capture) and manual image paste/drag |
